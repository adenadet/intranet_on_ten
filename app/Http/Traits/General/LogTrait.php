...
trait LogTrait{
    public function log_user_activity($type, $item_id, $status){
        switch ($type){
            case 'employee_leave_type_create':
                Activity::create([
                    'subject' => auth('api')->user()->first_name.' '.auth('api')->user()->last_name. ($status ? ' has successfully ' : 'unsuccessfully ').'assigned a leave type to an employee with ID: '.$item_id,
                    'url' => 'Employee Leave Type',
                    'method' => 'Create', 
