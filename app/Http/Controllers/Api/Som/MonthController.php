...
class MonthController extends Controller
{
    public function index()
    {
        return response()->json([
            ...
            'staff_month'    => Month::whereDate('voting_end', '<=', date('Y-m-d'))->with(['nominations.nominee', 'nomination_close', 'nomination_open', 'votes', 'voting_close', 'voting_open', 'winners.branch', 'winners.user.department'])->latest()->first(),
        ]);
    }
    ...
    {
        $month = Month::where('month', '=', $id.'-01')->with(['nominations.nominee', 'nomination_close', 'nomination_open', 'voting_close', 'voting_open',  'winners.branch', 'winners.user.department'])->first();
