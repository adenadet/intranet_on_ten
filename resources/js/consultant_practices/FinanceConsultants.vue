...
export default {
    ...
    methods:{
        ...
        getAllInitials(){
            this.loading = true;
            axios.get('/api/consultant_practices/consultants?type='+this.type+'&status='+this.status+'&start_date='+this.start_date+'&end_date='+this.end_date+'&query='+this.query)
            .then(response => {
                this.consultants = response.data.consultants ?? {data: [], total: 0,};
            })
            .catch(() => {
                this.$toast.fire({icon: 'error', title: 'Consultants did not loaded successfully',})
            })
            .finally(()=>{
                this.loading = false;
            });
